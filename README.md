...
├── images/hero-aquaman.jpg # фон-баннер «Аквамен» (используется как background-image)
...
- Фон-баннер «Аквамен» (`resources/images/hero-aquaman.jpg`) — реальное изображение
  из макета, подключено как `background-image`.
- Постеры фильмов (`public/images/posters/*.svg`) выполнены как **аккуратные
  SVG-плейсхолдеры** (векторные, локальные, масштабируемые), т.к. доступа к
  исходному файлу Figma не было. Для полной точности достаточно заменить эти файлы.
